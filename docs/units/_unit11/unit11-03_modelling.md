...
Statistical models can include many more variables and interactions. For example, in [species distribution modelling (SDM)](https://geomoer.github.io/moer-bsc-project-seminar-SDM/units.html), one might model species presence or abundance based on environmental predictors like temperature, precipitation, and forest cover. These models are often used to estimate potential current — and under environmental change, future — species distributions.
